...
graph <- tnum.makePhraseGraphFromPathList(tn.frame$subject, rootLabel="Subjects")

# Use the tnum rendering function to display it as a graph:

tnum.plotGraph(graph)
```
These plots render tnum paths in keeping with their intent as encodings of descriptive phrases.  In the phrase "wing of A-10 aircraft", or **aircraft:A-10/wing**, "A-10" functions as an adjective modifying noun "aircraft" in a rounded shape. The square node and solid arrow denote that binding of adjective and noun as an entity description.  The "wing of" or **/wing** is a possessive form, equivalent to "A-10's wing", shown as the dotted relationship arrow.  It's important to keep in mind that these encode descriptions, not specific structural relationships as a class or entity-relationship diagram would.  Think of these as explicit versions of what would normally be unstructured metadata like variable names or column names. 

### Adding knowledge with tags

The A-10 aircraft is nicknamed Warthog.  The F-35 is officially the Lightning-II, but Panther is gaining ground in the services.  Let's add this information to our tnums by applying appropriate tags.  While we're at it, we designate them as military aircraft:
```{r}
# add 2 tags to all A-10 tnums
tnum.tagByQuery("aircraft:A-10# has *",list("aircraft:military","nickname:official:Warthog"))

# and 3 tags to any F-35 tnums (we only have F-35C variant)
tnum.tagByQuery("aircraft:F-35* has *",list("aircraft:military","nickname:official:Lightning-II","nickname:unofficial:Panther"))
```
Now let's use a function that graphs tnum subjects, properties and tags together.  Because too many tags can clutter the picture, we have to provide a regular expression to select the tags we want to see.  Without that argument, no tags will be shown:
```{r}
# first re-query the database to get the new tags
tn.frame <- tnum.query("aircraft# has *")

# create the graph with only nickname tags shown
tgraph <- tnum.makeTnumPhraseGraph(tn.frame,tagMatch="^nickname.+")
tnum.plotGraph(tgraph,style="tree", size=500)
```
...
dftnums
```
Now we make and render a graph that includes the properties of the truenumbers and all tags:
```{r}
graph2 <- tnum.makeTnumPhraseGraph(dftnums, tagMatch = ".+")
...
```
That's quite a busy graph.  We can generate a summary graph by collecting on the terms in the subject paths by screen name and by tweet and profile id's:
```{r}
graph3 <- tnum.makeTnumPhraseGraph(dftnums, collectors = list("twitter/user:###/tweet:###","twitter/user:###/profile:###"))

tnum.plotGraph(graph3,size=800)
```
...
tweetTexts
```
Scroll right to see the sentiment tags in the tags column. We suppress display of neutral sentiment tags:

```{r}

graph4 <- tnum.makeTnumPhraseGraph(tweetTexts, tagMatch = "^analysis:sentiment:(positive|negative)")
tnum.plotGraph(graph4, size = 700)
```
This shows us that user AR_Yngve's 2 tweets have strong sentiment signals relative to the others.   
