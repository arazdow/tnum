...
**Truenumbers**, is a central cloud repository for creating, tagging and querying persistent data as a collection of self-describing facts represented in a pseudo-natural language. We call these facts *truenumbers*, *tnums* or *TNs*.  TNs mesh well with the way R allows descriptive attributes to be attached to data values, because R also recognizes the value of carrying descriptive informaton along with data values.  Using *tnum* provides a convenient and scalable way to manage data and results as a growing body of intelligible knowledge, without resorting to relational databases (which are usually far from intelligible).  
...
Next, we post the object list to ther server, then query the server for these tnums by subject path starting with *university#* , allowing any property, by using path and string wildcards **#** and **\***:
```{r}
...
graph <- tnum.makePhraseGraphFromPathList(returned.objects.frame$subject, rootLabel="Subjects")
...
tgraph <- tnum.makeTnumPhraseGraph(tn.objects, tagMatch = ".+")

tnum.plotGraph(tgraph, style = "neato", size = 750)
```
