...
tnum.authorize()
```
*authorize* can take a single argument, a string representing the URL or IP address of the *tnum* server endpoint.  The default will be the endpoint for your particular installation.
...
```
Now we query the tnums into a data.frame with a simple query for the custom tag we added to them.
```{r}
dftnums <- tnum.query("@project:seti:tweets:1")
dftnums
```
